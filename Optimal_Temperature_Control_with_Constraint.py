...
arg_pass.add_argument(
  "--e",
  help='Total episodes to train through all environments; default 8000',
  type=int,  
  default=8000,
)
arg_pass.add_argument(
...
arg_pass.add_argument(
  "--E",
  help='Episode in which the beta is zero; default 4000',
  type=int,  
  default=4000,
)
arg_pass.add_argument(
...
en= 'Temperature_Control' 
Constraint_Based_Termination= False
Generate_Obs_in_Valid_Range= False

print('Env Name:', 'Temperature_Control','\nNumber of total episodes:', n_EPISODES,
      ...
      '\nSave_results=', './Results/Temperature_Control/', '\nDebug:', Debug)


class Control(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, constraint_based_termination= False, generate_obs_in_valid_range= False, n_state=3, n_action=3, 
        ...
        self.action_space = gym.spaces.Box(low=-1, high=1, shape=(self.n_action,), dtype=np.float32)
...
#                     self.writer.add_scalar(f'Workers:{1}/o_average_score',  o_average, self.episode)       
        ...
        data.to_csv(self.save_results+'Temperature_Control_e{}_omega{}_beta{}_E{}.csv'.format(self.EPISODES, self.a0, self.initial_beta, self.cut), index = False)
        print("The results saved in:"+Save_results+'Temperature_Control_e{}_omega{}_beta{}_E{}.csv'.format(self.EPISODES, self.a0, self.initial_beta, self.cut))
